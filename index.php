...
<body>
    <section id="app_table">
        ...
        <table>
            ...
            <tbody>
                <tr v-for="items in registros">
                    <td>{{ items.no_carga }}</td>
                    <td>{{ items.fecha }} </td>
                    <td>{{ items.no_rutas }} </td>
                    <td>{{ items.rutas_entregar }} </td>
                    <td>{{ items.vehiculo_id }} </td>
                    <td>{{ items.tipo }} </td>
                    <td>{{ items.marca }} </td>
                    <td>{{ items.capacidad_kgs }} </td>
                    <td>{{ items.no_partidas }} </td>
                    <td>{{ items.partidas_escaneadas }} </td>
                    <td>{{ items.p_escaneado_partidas }} </td>
                    <td>{{ items.total_kgs }} </td>
                    <td>{{ items.total_kgs_escaneados }} </td>
                    <td>{{ items.p_escaneado_kgs }} </td>
                    <td>{{ items.no_clientes }} </td>
                    <td>{{ items.no_pedidos }} </td>
                    <td>{{ items.estatus }} </td>
        ...
        let app = new Vue({
            ...
            data: {
                registros: [
                    ...
                    {no_carga:101, fecha:'15/02/2023', no_rutas:1, rutas_entregar:'R10 MARAVATIO', vehiculo_id:18, tipo:'CAMIONETA', marca:'FORD F-350', capacidad_kgs:3500, no_partidas:35, partidas_escaneadas:1, p_escaneado_partidas:3, total_kgs:2127.74, total_kgs_escaneados:110, p_escaneado_kgs:5, no_clientes:7, no_pedidos:8, estatus:'CARGADO', ult_actualizacion: '16/02/23 10:56:43 a.m.'},
                    {no_carga:95, fecha:'15/02/2023', no_rutas:2, rutas_entregar:'R11 ZACATECAS \n R85 AGUASCALIENTES', vehiculo_id:13, tipo:'CAMION TORTON', marca:'KENWORTH', capacidad_kgs:15000, no_partidas:48, partidas_escaneadas:0, p_escaneado_partidas:0, total_kgs:14515.67, total_kgs_escaneados:0, p_escaneado_kgs:0, no_clientes:8, no_pedidos:9, estatus:'EN CARGA', ult_actualizacion: '16/02/23 10:56:43 a.m.'},
                    ...
                    {no_carga:80, fecha:'15/02/2023', no_rutas:2, rutas_entregar:'R21 LEON \n R23 SILAO', vehiculo_id:44, tipo:'CAMIONETA', marca:'NISSAN NP 300', capacidad_kgs:1500, no_partidas:14, partidas_escaneadas:11, p_escaneado_partidas:76, total_kgs:1851.40, total_kgs_escaneados:826.40, p_escaneado_kgs:45, no_clientes:3, no_pedidos:3, estatus:'CARGADO', ult_actualizacion: '16/02/23 10:56:43 a.m.'},
                    ...
                    {no_carga:74, fecha:'15/02/2023', no_rutas:3, rutas_entregar:'R14 ACAPULCO \n R16 LAZARO CARDENAS \n R87 APATZINGAN', vehiculo_id:1002, tipo:'GENERICA', marca:'NA (BODEGA)', capacidad_kgs:35000, no_partidas:48, partidas_escaneadas:18, p_escaneado_partidas:38, total_kgs:16153.30, total_kgs_escaneados:14694.86, p_escaneado_kgs:91, no_clientes:20, no_pedidos:20, estatus:'CARGADO', ult_actualizacion: '16/02/23 10:56:43 a.m.'},
                ],
            },
            created(){
            },
